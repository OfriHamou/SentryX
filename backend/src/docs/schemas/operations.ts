...
export const operationSchemas: Record<string, OpenApiObject> = {
    ...
    AdminAlert: {
        type: "object",
        required: ["id", "status", "displayTitle", "createdAt", "updatedAt", "tenant"],
        description: "A platform-wide Alert projection. Only explicitly serialized Tenant, Event, Robot, assignment, and resolution fields are exposed.",
        properties: {
            id: uuid,
            status: { type: "string", enum: ["OPEN", "IN_PROGRESS", "RESOLVED"] },
            ...
            startedAt: nullableDateTime,
            resolvedAt: nullableDateTime,
            resolutionNotes: { type: "string", nullable: true, maxLength: 1000 },
            createdAt: dateTime,
            updatedAt: dateTime,
            tenant: { $ref: "#/components/schemas/AdminAlertTenant" },
            assignedUser: { allOf: [{ $ref: "#/components/schemas/AlertAssignedUser" }], nullable: true },
            assignedShift: { allOf: [{ $ref: "#/components/schemas/AlertAssignedShift" }], nullable: true },
            resolvedBy: { allOf: [{ $ref: "#/components/schemas/AdminAlertResolvedBy" }], nullable: true },
            event: { allOf: [{ $ref: "#/components/schemas/Event" }], nullable: true },
        },
    },
    AdminAlertCounts: {
        type: "object",
        required: ["all", "open", "inProgress", "active", "resolved", "tenantsWithActive"],
        description: "Counts preserve tenant, date, and search filters while ignoring the selected status tab.",
        properties: {
            all: { type: "integer", minimum: 0 },
            open: { type: "integer", minimum: 0 },
            inProgress: { type: "integer", minimum: 0 },
            active: { type: "integer", minimum: 0, description: "OPEN plus IN_PROGRESS." },
            resolved: { type: "integer", minimum: 0 },
            tenantsWithActive: { type: "integer", minimum: 0 },
        },
    },
    AdminAlertListResponse: {
        ...
        properties: {
            ok: { type: "boolean", example: true },
            alerts: { type: "array", items: { $ref: "#/components/schemas/AdminAlert" } },
            counts: { $ref: "#/components/schemas/AdminAlertCounts" },
            pagination: { $ref: "#/components/schemas/Pagination" },
        },
    },
