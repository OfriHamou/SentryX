...
interface Props { face: AuthorizedFace; open: boolean; onClose: () => void; onChanged: () => void; canWrite: boolean; }

export default function FaceModal({ face, open, onClose, onChanged, canWrite }: Props) {
        ...
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            ...
            <DialogContent>
                <Stack spacing={2} sx={{ mt: 1 }}>
                    <TextField label="Name" value={name} onChange={(e) => setName(e.target.value)} size="small" fullWidth disabled={!canWrite} />
                    <TextField label="Role" value={role} onChange={(e) => setRole(e.target.value)} size="small" fullWidth disabled={!canWrite} />
            ...
            <DialogActions>
                ...
                {canWrite && <Button onClick={handleSave} startIcon={<SaveIcon />} variant="contained" disabled={busy} sx={{ textTransform: 'none' }}>Save</Button>}
            </DialogActions>
        </Dialog>
    );
}
