...
import { Box, Paper, Typography, Grid, Dialog, Chip, Stack, Select, MenuItem, TextField, FormControl } from '@mui/material';
    ...
    return (
        <Box>
            ...
            {timeRange === 'custom' && (
                <Stack direction="row" spacing={2} sx={{ mb: 3 }}>
                    <TextField type="date" size="small" label="From" value={customFrom}
                        onChange={(e) => setCustomFrom(e.target.value)} slotProps={{ inputLabel: { shrink: true } }} sx={{ bgcolor: '#fff', borderRadius: 2 }} />
                    <TextField type="date" size="small" label="To" value={customTo}
                        onChange={(e) => setCustomTo(e.target.value)} slotProps={{ inputLabel: { shrink: true } }} sx={{ bgcolor: '#fff', borderRadius: 2 }} />
                </Stack>
            )}

            {withImages.length === 0 ? (
                <Typography color="text.secondary">No frames captured yet</Typography>
            ) : (
                <Grid container spacing={2}>
                    {withImages.map((e) => {
                        const display = getEventDisplay(e);
                        return (
                            <Grid size={{ xs: 6, sm: 4, md: 3 }} key={e.id}>
                                <Paper elevation={0} onClick={() => setSelected(e)}
                                    sx={{ borderRadius: 2, overflow: 'hidden', cursor: 'pointer', border: '1px solid', borderColor: 'grey.200', '&:hover': { boxShadow: 3 } }}>
                                    <Box component="img" src={eventDbImageUrl(e.id)} alt={display.title(e)}
                                        sx={{ width: '100%', height: 140, objectFit: 'cover', display: 'block', bgcolor: 'grey.100' }} />
                                    <Box sx={{ p: 1 }}>
                                        <Typography variant="caption" sx={{ fontWeight: 600, display: 'block', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                            {display.title(e)}
                                        </Typography>
                                        <Typography variant="caption" color="text.disabled">{new Date(e.timestamp).toLocaleString()}</Typography>
                                    </Box>
                                </Paper>
                            </Grid>
                        );
            ...
            <Dialog open={!!selected} onClose={() => setSelected(null)} maxWidth="md">
                {selected && (
                    <Box>
                        <Box component="img" src={eventDbImageUrl(selected.id)} alt=""
                            sx={{ width: '100%', maxHeight: '70vh', objectFit: 'contain', display: 'block', bgcolor: 'black' }} />
                        <Stack direction="row" spacing={2} sx={{ p: 2, alignItems: 'center' }}>
                            <Typography sx={{ fontWeight: 700 }}>{getEventDisplay(selected).title(selected)}</Typography>
                            {selected.is_alert && <Chip label="Alert" size="small" color="error" />}
                            <Box sx={{ flexGrow: 1 }} />
                            <Typography variant="caption" color="text.secondary">{new Date(selected.timestamp).toLocaleString()}</Typography>
                        </Stack>
                    </Box>
                )}
            </Dialog>
