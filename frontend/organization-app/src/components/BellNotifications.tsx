...
import UnreadIcon from '@mui/icons-material/Circle';
import {
  getNotifications,
        ...
        <Stack spacing={2}>
          ...
          {notification?.metadata && (
            <Box>
              ...
              <Box component="pre" sx={{ m: 0, p: 1.5, bgcolor: '#F8F5FF', borderRadius: 1, overflow: 'auto', fontSize: 12 }}>
  ...
  return (
    <>
      <Tooltip title="Notifications">
        <IconButton onClick={handleOpen} sx={{ color: '#5B21B6', width: 38, height: 38, borderRadius: '8px' }} aria-label="Open notifications">
      ...
      <Popover open={open} anchorEl={anchorEl} onClose={() => setAnchorEl(null)} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} transformOrigin={{ vertical: 'top', horizontal: 'right' }}>
        <Box sx={{ width: 360, maxWidth: 'calc(100vw - 32px)' }}>
          <Stack direction="row" sx={{ alignItems: 'center', justifyContent: 'space-between', p: 2 }}>
            <Box>
              <Typography sx={{ fontWeight: 850, color: '#20113E' }}>Notifications</Typography>
          ...
          {loading && notifications.length === 0 ? (
            <Box sx={{ p: 3, display: 'grid', placeItems: 'center' }}><CircularProgress size={24} /></Box>
            ...
            <List disablePadding sx={{ maxHeight: 420, overflow: 'auto' }}>
              {notifications.map((notification) => (
                <ListItemButton key={notification.id} onClick={() => void handleNotificationClick(notification)} sx={{ alignItems: 'flex-start', gap: 1.25, py: 1.5, bgcolor: notification.isRead ? 'transparent' : '#F1EBFF' }}>
                  <Box sx={{ pt: 0.4, color: notification.isRead ? 'text.disabled' : '#7C3AED' }}>
