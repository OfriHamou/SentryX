...
const TARGET_APP = 'ADMIN' as const;
        ...
        <Stack spacing={2}>
          ...
          {notification?.metadata && (
            <Box>
              ...
              <Box component="pre" sx={{ m: 0, p: 1.5, bgcolor: '#F4F7FE', borderRadius: 1, overflow: 'auto', fontSize: 12 }}>
  ...
  return (
    <>
      <Tooltip title="Notifications">
        <IconButton onClick={handleOpen} sx={{ color: '#A3AED0', width: 40, height: 40 }} aria-label="Open notifications">
      ...
      <Popover open={open} anchorEl={anchorEl} onClose={() => setAnchorEl(null)} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} transformOrigin={{ vertical: 'top', horizontal: 'right' }}>
        <Box sx={{ width: 360, maxWidth: 'calc(100vw - 32px)' }}>
          <Stack direction="row" sx={{ alignItems: 'center', justifyContent: 'space-between', p: 2 }}>
            <Box>
              <Typography sx={{ fontWeight: 800, color: '#2B3674' }}>Notifications</Typography>
          ...
          {loading && notifications.length === 0 ? (
            ...
            <List disablePadding sx={{ maxHeight: 420, overflow: 'auto' }}>
              {notifications.map((notification) => (
                <ListItemButton key={notification.id} onClick={() => void handleNotificationClick(notification)} sx={{ alignItems: 'flex-start', gap: 1.25, py: 1.5, bgcolor: notification.isRead ? 'transparent' : '#FDECEB' }}>
                  <Box sx={{ pt: 0.4, color: notification.isRead ? 'text.disabled' : '#EE5D50' }}>
