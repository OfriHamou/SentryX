...
export const markNotificationRead = async (id: string) => {
  const response = await api.patch<{ notification: BellNotification }>(`/notifications/${id}/read`);
  return response.data.notification;
};

export const markNotificationUnread = async (id: string) => {
  const response = await api.patch<{ notification: BellNotification }>(`/notifications/${id}/unread`);
  return response.data.notification;
};
